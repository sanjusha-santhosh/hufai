...
>  ⚠️ HufAI is actively being migrated from an existing implementation into an independent app. The system may not work as expected and is not recommended for use in production environments at this stage. ⚠️ 
...
bench get-app https://github.com/Tridz/hufai.git
bench install-app hufai
...
HufAI is built around a set of interconnected DocTypes that define the components of an AI agent. The core logic is handled by Python classes that integrate with an AI provider (like OpenAI) and manage the agent's lifecycle.
...
3.  **Tool Serialization**: The `sdk_tools.py` module converts the HufAI tools into a format that the AI provider's SDK can understand.
