...
The application is built on the Frappe Framework (Python) and uses the standard Frappe directory structure. The core logic for agent integration is located in `hufai/ai/`.

## Repo Layout
-   `hufai/`: The root of the Frappe app.
-   `hufai/hufai/`: The main Python module.
-   `hufai/hufai/doctype/`: Contains all DocType definitions, each in its own folder.
...
-   `hufai/ai/`: Core Python modules for AI agent integration.
...
-   **LiteLLM Dependency**: The `litellm` package (>=1.0.0) is required and listed in `pyproject.toml`. It will be automatically installed when running `bench setup requirements`. The post-install hook (`hufai/install.py`) checks for LiteLLM and provides guidance if it's missing.
...
1.  **Provider & Model**: You start by defining an `AI Provider` (e.g., OpenAI, Anthropic, Google) and the `AI Model` you want to use (e.g., `gpt-4-turbo`, `claude-3-opus`). HufAI uses LiteLLM to provide unified access to 100+ LLM providers through a single interface.
...
Here is a detailed breakdown of the DocTypes used in HufAI.

#### 1. AI Provider

Stores credentials for different AI service providers. HufAI uses LiteLLM to provide unified access to 100+ LLM providers, including OpenAI, Anthropic, Google, OpenRouter, xAI (Grok), Mistral, and many more.

-   **Python Class**: `AIProvider(Document)`
-   **File**: `hufai/hufai/doctype/ai_provider/ai_provider.py`
...
-   **File**: `hufai/hufai/doctype/ai_model/ai_model.py`
...
-   **File**: `hufai/hufai/doctype/agent_tool_function/agent_tool_function.py`
...
-   **File**: `hufai/hufai/doctype/agent/agent.py`
...
-   **File**: `hufai/hufai/doctype/agent_conversation/agent_conversation.py`
...
-   **File**: `hufai/hufai/doctype/agent_message/agent_message.py`
...
-   **File**: `hufai/hufai/doctype/agent_run/agent_run.py`
...
-   **File**: `hufai/hufai/doctype/agent_chat/agent_chat.py`

**Features:**
-   Real-time chat UI with markdown rendering
-   Message history display
-   Only available for agents with `enable_chat` enabled
-   Server Actions: `hufai.ai.agent_chat.get_agent_chat_messages`, `hufai.ai.agent_chat.send_agent_chat_message`

### Core Classes and Methods

The primary logic is located in the `hufai/ai` directory.
...
This file acts as a bridge between HufAI's `Agent Tool Function` DocType and the `agents` SDK's `FunctionTool` class.
